...
Zero-infrastructure philosophy (same as review.html): one file, inline CSS, no
JS libraries, no network. Opens in any browser.

Visual system (dataviz skill, reference palette):
- Heatmap magnitude uses the sequential blue ramp, one hue light→dark; dark mode
  is its own selected steps (low values recede toward the dark surface).
- The strand chart is a single series → slot-1 blue, thin bars, 4px rounded
  data-end, value at the tip in a text token; no legend (title names the series).
- All text wears text tokens, never the series color. Hairline rules only.
- Every chart has a table-view twin; heatmap cells carry native tooltips.
"""
...
# Sequential blue bins (reference palette): light mode low→high = light→dark;
# dark mode is a selected reversal so low recedes toward the dark surface.
# (bg-light, ink-light, bg-dark, ink-dark)
_BINS = [
    ("#cde2fb", "#0b0b0b", "#0d366b", "#ffffff"),
    ("#b7d3f6", "#0b0b0b", "#104281", "#ffffff"),
    ("#9ec5f4", "#0b0b0b", "#184f95", "#ffffff"),
    ("#6da7ec", "#0b0b0b", "#256abf", "#ffffff"),
    ("#3987e5", "#ffffff", "#3987e5", "#ffffff"),
    ("#1c5cab", "#ffffff", "#5598e7", "#0b0b0b"),
    ("#0d366b", "#ffffff", "#86b6ef", "#0b0b0b"),
]
...
.hm th.col{writing-mode:initial;text-align:center}
...
.hm td:hover{outline:2px solid var(--ink);outline-offset:-2px}
...
/* bars */
.bars .row{display:grid;grid-template-columns:220px 1fr 52px;align-items:center;
  gap:10px;padding:5px 0}
.bars .row:hover{background:color-mix(in srgb,var(--ink) 4%,transparent)}
...
def _cell(pct: float | None, title: str) -> str:
    if pct is None:
        return f'<td class="na" title="{_esc(title)}: not assessed">—</td>'
    b = _bin(pct)
    return (
        f'<td class="b{b}" title="{_esc(title)}: {pct:.0f}%">{pct:.0f}</td>'
    )
...
def render_report(result: AttainmentResult, pack: CurriculumPack) -> str:
    ...
    # ---- Heatmap: rows = cohort + students, cols = outcome codes -------------
    head = "<tr><th></th>" + "".join(
        f'<th class="col" title="{_esc(outcomes[c].description)}">{_esc(c)}</th>' for c in codes
    ) + "</tr>"
    cohort_row = '<tr class="cohort"><th>Cohort</th>' + "".join(
        _cell(_pct(*result.cohort[c]), f"Cohort · {c}") for c in codes
    ) + "</tr>"
    student_rows = "".join(
        f"<tr><th>{_esc(sid)}</th>"
        + "".join(
    ...
    ) + "<span>100%</span><span style=\"margin-left:12px\">— = not assessed</span></div>"

    # Table-view twin (accessibility: values without color)
    ...
    table_twin = (
        ...
        '<table class="plain"><tr><th>Student</th><th>Outcome</th><th>Awarded</th>'
        "<th>Available</th><th>%</th></tr>" + table_twin_rows + "</table></details>"
    )

    # ---- Strand / dimension bars ---------------------------------------------
    def bar_rows(data: dict[str, list[float]]) -> str:
        ...
        for name, (aw, av) in sorted(data.items(), key=lambda kv: -(_pct(*kv[1]) or 0)):
            ...
            if pct is None:
                continue
            rows.append(
                f'<div class="row"><div class="name" title="{_esc(name)}">{_esc(name)}</div>'
    ...
    unassessed_html = (
        "".join(
            f'<span class="pill" title="{_esc(outcomes[c].description)}">{_esc(c)}</span>'
...
Every figure is marks earned vs marks available on questions tagged to that outcome.</p>
{untagged_note}
